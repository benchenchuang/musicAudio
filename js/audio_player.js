...
;(function($){
    ...
    (function getData() {
        ...
        $.ajax({
            ...
            error:function(){
                ...
                $lyricList.html('<li style="height:auto;color: red; padding: 0.5rem; text-align: left ">获取歌曲列表失败，请联系管理员！</li>');
            }
        });
    ...
    function getTime() {
        setTimeout(function () {
            isNaN(music.duration)?getTime():$timeLong.html(getDouble(music.duration));
        });
    }
    /*播放*/
    ...
    function iplay() {
        ...
        (music.onloadeddata = function () {         /* loadeddata 当浏览器已加载音频/视频的当前帧时触发。*/
            ...
            $timeLong.html(getDouble(music.duration));  /*加载歌曲时长*/
        })();
        music.play();
        isPlay = true;
    ...
    music.ontimeupdate = function () {
        ...
        $scale.val(value);
        $timeCur.html(getDouble(value));/*加载载歌曲当前播放时间*/
